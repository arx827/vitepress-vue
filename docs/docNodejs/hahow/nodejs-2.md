...
## 單元 6 - NPM 專案管理工具
  ...
  - #### 管理第三方模組
    ...
    - 專案依賴多個第三方模組
    - 第三方模組又依賴其他模組
    - 依賴上百個模組，是我們開發專案常見的狀況

  ### NPM 專案管理工具 (Node Package Manager)
  - #### NPM 安裝
    內建在 `Node.js` 的安裝程序中
    - 安裝 `Node.js` 時，就已經安裝好 `NPM 專案管理工具`
...
## 單元 7 - NPM 第三方模組管理
  ![nodejs-2-10](/docNodejs/nodejs-2-10.png)
  ...
  - #### 解除安裝第三方模組
    ...
    - 使用語法
      ```sh
      npm uninstall 第三方模組名稱
      ```
    - 第三方模組程式會從<br>
      `node_modules` 子資料夾中刪除
    - 專案描述檔中會移除已經解除安裝的模組紀錄

  - #### 實作：初始化、模組管理命令練習
    > 沒有初始化的專案(沒有 package.json)，無法安裝第三方模組
    - 初始化專案
      ```sh
      npm init -y
      ```
